...
vtolthrust = vtolthrust*safety_factor*n_vtol
...
for loadcase in loadcasearray: 
    ...
    nbottomstring = 0
    ntopstring = 0
    l4st,l2st,l1st,l3st,thetat2,thetab2 =  wingboxdimension(airfoil,frontspar,rearspar)
    
##########################################################################
#ALL OF THE REST NEEDS TO BE INDENTED ONCE MORE
##########################################################################
...
for force in loadcase:
    ...
    #### tap   
        ...
        Izz = mom_of_inertia(c,l1,l2,l3,l4,alpha,beta,t1,t2,t3,t4,t_s,h_s)[3]
        Ixx = mom_of_inertia(c,l1,l2,l3,l4,alpha,beta,t1,t2,t3,t4,t_s,h_s)[2]
        Ixz = mom_of_inertia(c,l1,l2,l3,l4,alpha,beta,t1,t2,t3,t4,t_s,h_s)[4]

        x_centroid_box = mom_of_inertia(c,l1,l2,l3,l4,alpha,beta,t1,t2,t3,t4,t_s,h_s)[0]
        y_centroid_box = mom_of_inertia(c,l1,l2,l3,l4,alpha,beta,t1,t2,t3,t4,t_s,h_s)[1]
        ...
        x_sc,y_sc,tau1,tau1_loc,tau2,tau2_loc,tau3,tau3_loc,tau4,tau4_loc,tau5,tau5_loc,tau6,tau6_loc,tau7,tau7_loc,tau8,tau8_loc,twist = shear_stress(t1,t2,t3,t4,l1,l2,l3,h4,thethat2,thethab2,width,x_centroid_box,(hf-y_centroid_box),x_centroid_box,(hf-y_centroid_box),Ixx,Izz,Ixz,G,Vy,Vx,x_Vy,z_Vx)
        ...
        sigma1,sigma1_loc,sigma2,sigma2_loc,sigma3,sigma3_loc,sigma4,sigma4_loc,sigma5,sigma5_loc,sigma6,sigma6_loc,sigma7,sigma7_loc,sigma8,sigma8_loc,=   normal_stress(t1,t2,t3,t4,l1,l2,l3,l4,thethat2,thethab2,width,x_centroid_box,(hf-y_centroid_box),x_centroid_box,(hf-y_centroid_box),Ixx,Izz,Ixz,Mz,Mx)
        #1,2,3,4 max stresses on webs 1,2,3,4
        #5,6,7,8 min stresses on webs 1,2,3,4
        #loc gives position of max/min stress as A POSITION IN METERS w.r.t the web

        #TopsKing
        CCstress = stiffendskincalculation(chord,x,ntopstring,E,v)
        while CCstress < sigma5:
            ...
            if ntopstring > 10:
                ...
                break
        #Bottomsking
        CCstress = stiffendskincalculation(chord,x,nbottomstring,E,v)
        while CCstress < sigma7:
            ...
            if nbottomstring > 10:
                print("BREAK CCSTRESS")
                break    
        dstring = - nbottomstrin - ntopstring
